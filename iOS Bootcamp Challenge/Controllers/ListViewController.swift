...
class ListViewController: UICollectionViewController {
    ...
    @objc func refresh() {
        ...
        PokeAPI.shared.get(url: "pokemon?limit=30", onCompletion: { [weak self] (list: PokemonList?, _) in
            guard let list = list else { return }
            list.results.forEach { result in
                dispatchGroup.enter()
                PokeAPI.shared.get(url: "/pokemon/\(result.id)/", onCompletion: { [weak self] (pokemon: Pokemon?, _) in
                    defer{
                        dispatchGroup.leave()
                    }
